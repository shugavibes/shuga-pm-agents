...
import { getSlackToken } from './slack-token.mjs';
...
async function slackPost(params) {
  // Direct Slack Web API for posting — chat:write is in the token scopes and
  // avoids the MCP tool's block-rendering failures on long/complex messages.
  const token = getSlackToken();
...
export async function postSlackMessage(channelId, text) {
  return slackPost({ channel: channelId, text, mrkdwn: 'true' });
}

export async function postSlackReply(channelId, threadTs, text) {
  return slackPost({ channel: channelId, text, mrkdwn: 'true', thread_ts: threadTs });
}
