...
function getCursorDbPath() {
  ...
  switch (process.platform) {
    case 'darwin': return join(home, 'Library/Application Support/Cursor/User/globalStorage/state.vscdb');
    case 'win32':  return join(home, 'AppData/Roaming/Cursor/User/globalStorage/state.vscdb');
    default:       return join(home, '.config/Cursor/User/globalStorage/state.vscdb'); // Linux
  }
}
const CURSOR_DB = getCursorDbPath();
...
function readFromCursor() {
  ...
  try {
    if (process.platform !== 'darwin') return null; // Keychain decryption is macOS-only
